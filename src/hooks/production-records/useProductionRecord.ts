...
export const useProductionRecordsByDate = (startDate: Date, endDate: Date) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await api.get<ApiResponse<ProductionRecord[]>>(`/production-records?${params}`)
      return response.data
    },
  })
}
...
export const useProductionRecordById = (id: number | undefined) => {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<ApiResponse<ProductionRecord>>(`/production-records/${id}`)
      return response.data
    },
    enabled: !!id
  })
...
export const useUpdateProductionRecordById = () => {
  ...
  return useMutation({
    mutationFn: async (record: Partial<ProductionRecord> & { id: number }) => {
      const response = await api.put<ApiResponse<ProductionRecord>>(
...
export const useDeleteProductionRecordById = () => {
  ...
  return useMutation({
    mutationFn: async (id: number) => {
      const response = await api.delete<ApiResponse<ProductionRecord>>(`/production-records/${id}`)
      return response.data
    },
    onSuccess: (_, id) => {
      queryClient.invalidateQueries({ queryKey: queryKeys.all })
...
export const useDeleteProductionRecordsById = () => {
  ...
  return useMutation({
    mutationFn: async (ids: number[]) => {
      const response = await api.delete<ApiResponse<string>>('/production-records/batch', {
        data: { ids }
      })
      return response.data
    },
...
export const useCreateProductionRecord = () => {
  ...
  return useMutation({
    mutationFn: async (newRecord: Omit<ProductionRecord, 'id' | 'created_at' | 'updated_at'>) => {
      const response = await api.post<ApiResponse<string>>('/production-records', newRecord)
      return response.data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.all })
